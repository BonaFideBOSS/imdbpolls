...
for i in data["polls"]:
    ...
    try:
        ...
        if ("inactive" not in inactivecheck) & ("closed" not in closedcheck):
            ...
            xxdate = scrapeMob.select_one(".btn-full .media .media-body .media-heading")
            if polldate:
                ...
                if (":" not in polldate) & ("ago" not in polldate):
                    ...
                    if datetime.strptime(polldate, "%Y/%m/%d") > datetime.now():
                        polldate = str(datetime.today().year - 1) + datetime.strptime(
                            polldate, "%Y/%m/%d"
                        ).strftime("/%m/%d")
                elif "days ago" in polldate | "day ago" in polldate:
                    polldate = polldate[:1]
                    polldate = (datetime.today() - timedelta(int(polldate))).strftime(
                        "%Y/%m/%d"
                    )
                elif "hours ago" in polldate | "hour ago" in polldate:
